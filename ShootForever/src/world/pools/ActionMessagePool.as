/**
 *
 * Adapted (with love & appreciation) from Hugry Hero's object pool template:
 * 
 * Hungry Hero Game
...
package world.pools
{
	...
	/** A pool of ActionMessage objects */
	public class ActionMessagePool {
		...
		/** Objects in the pool. */
		private var list:Vector.<ActionMessage> = new Vector.<ActionMessage>();
		...
		public function ActionMessagePool(create:Function, clean:Function = null, minSize:int = 50, maxSize:int = 200)
		{
			this.create = create;
			this.clean = clean;
		 ...
		 *
		 */
		public function checkOut():scenes.ui.ActionMessage
		{
			if(length == 0) {
				if(size < maxSize) {
					size++;
		 ...
		 *
		 */
		public function checkIn(item:ActionMessage):void
		{
