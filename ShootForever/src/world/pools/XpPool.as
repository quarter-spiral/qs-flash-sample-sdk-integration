...
package world.pools
{
	import world.XpObject;
	
	/** A pool of xp objects */
	public class XpPool {
		...
		/** Objects in the pool. */
		private var list:Vector.<XpObject> = new Vector.<XpObject>();
		...
		public function XpPool(create:Function, clean:Function = null, minSize:int = 50, maxSize:int = 200)
		{
			this.create = create;
			this.clean = clean;
		 ...
		 *
		 */
		public function checkOut():XpObject
		{
			if(length == 0) {
				if(size < maxSize) {
					size++;
		 ...
		 *
		 */
		public function checkIn(item:XpObject):void
		{
